# day 11 - monkey in the middle, pt2 (huge nubers)
# answer=14636993466
...
def k(m): #newline#space #do a round for monke m
  for old in m[0]: # for each item in this monke's posession (assigned to 'old')
    n=eval(m[1])%prod(m[2]for m in M); #do the new=f(old) expression, loop at P
    d = int(m[4]if n%m[2]else m[3]); #work out where this item is going based on divisibility
    M[d][0]+=[n]; #move it there
...
#product of all denominators; take modulo of this to keep nums small. (something something equivalence classes)
# P=prod(m[2]for m in M);
# inlined to avoid variable assignment; RIP runtime efficiency :')


# for i in range(10000):
#   print(i,M)
#   [k(m) for m in M]
[k(m) for i in [0]*10000 for m in M]; #run 10k rounds

print(prod(sorted(m[5]for m in M)[-2:]))
